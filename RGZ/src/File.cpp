...
istream &operator>>(istream &in, File &file)
{
    ...
    if (regex_match(ss.str(), reg))
    {
        ...
        file.SetReadPermission((tmp_read == "true") ? true : false);
        file.SetWritePermission((tmp_write == "true") ? true : false);
        file.SetExecutePermission((tmp_execute == "true") ? true : false);
