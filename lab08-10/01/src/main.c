/**
 * @file main.c
 * @brief Файл з реалізацією і викликом функції.
 *
 * @author Babenko A.
 * @date 29-nov-2020
 * @version 1.0
 */

/**
 * Визначення найбільшого спільного дільника
 * 
 * Функція, що визначає найбільший спільний дільник для двох заданих чисел
 *	@param a одне з чисел для знаходження нсд
 *	@param b псевдовипадкове число для знаходження нсд
 *  @return Результат роботи функції (res) 
 */
int nod(int, int);

/**
 * Отримання псевдовипадкового числа
 * 
 * Функція, що генерує і повертає псевдовипадкове число
 *  @return Псевдовипадкове число (random) 
 */
int get_rand();

/**
 * Головна функція
 * 
 * Послідовність дій:
 * - виклик функції srand
 * - виклик функції nod
 * @return успішний код виконання програми (0)
 */
int main()
{
  srand(time(0));

  // printf("%d\n", nod(6, get_rand()));
  nod(6, get_rand());
  return 0;
}

int nod(int a, int b)
{
  /* Объявляем переменные для записи результата */
  int res;
  int i = 1;

  /* Запись младшего числа в переменную b */
  if (a < b)
  {
    int x = a;
    a = b;
    b = x;
  }

  /* Цикл прохождения от 1 до младшего числа */
  for (i = 1; i <= b; i++)
  {
    /* Если остаток от деления двух чисел == 0, записываем его в переменную */
    if ((a % i) == 0 && (b % i) == 0)
    {
      res = i;
    }
  }

  /*
  while (i <= b)
  {
    if ((a % i) == 0 && (b % i) == 0) 
    {
      res = i;
    }
    i++;
  }
*/

  /*
  do {
    if ((a % i) == 0 && (b % i) == 0) 
    {
      res = i;
    }
    i++;
  } while (i <= b);
*/

  return res;
}

int get_rand() {
  int random = rand() % 10 + 1; 
  return random;
}
